...
- **Paused**: `GameConfigEmitter` and `ConfigAccessorEmitter` are commented out in `FlockCodegenMenu` while GameConfig codegen is being aligned with the new typed-schema shape. `Flock > Sync Schemas` regenerates Templates, Player accessors, Commands, and the Manifest; the `Configs/` subdirectory is left alone until GameConfig is re-enabled.
