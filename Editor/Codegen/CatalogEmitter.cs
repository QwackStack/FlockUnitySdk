...
namespace Flock.Editor.Codegen
{
    ...
    internal static class CatalogEmitter
    {
        ...
        public static int Emit(FlockSchemaSnapshot snapshot, string gameVersionName, string generatedRoot)
        {
            ...
            Debug.Log($"[Flock Codegen] Catalog: {catalog.shops.Count} shop(s), {catalog.configs.Count} config(s), {catalog.templates.Count} template(s) → {assetPath}");
            return catalog.shops.Count + catalog.configs.Count + catalog.templates.Count;
        }
