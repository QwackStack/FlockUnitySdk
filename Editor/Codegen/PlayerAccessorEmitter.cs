...
namespace Flock.Editor.Codegen
{
    ...
    internal static class PlayerAccessorEmitter
    {
            ...
            string outputDir)
        {
            ...
            // Sorted: UnDuplicate hands out Foo / Foo_2 in iteration order, so a reordered response would rebind
            // an accessor. SchemaHasher sorts too, so ContentHash wouldnt change and Verify would see no drift.
            foreach (PlayerTemplateSchema t in (playerTemplates ?? new List<PlayerTemplateSchema>())
                         .Where(x => x != null)
                         .OrderBy(x => x.Id ?? "", StringComparer.Ordinal)
                         .ThenBy(x => x.Name ?? "", StringComparer.Ordinal))
            {
                if (string.IsNullOrEmpty(t.Id) || string.IsNullOrEmpty(t.Name)) continue;
